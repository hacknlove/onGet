 ...
 *  refresh('/api/user/current')
 */
function afterRefetch (path, hook) {
...
function propagateUp (url) {
  ...
  if (resource) {
    resource.value = getValue(state$1, resource.url);
    Object.values(resource.callbacks).forEach(cb => cb(resource.value));
  }
...
var dotted = {
   ...
   */
  getResource (resource) {
    const actualValue = getValue(state$1, resource.url);
    if (actualValue === undefined) {
      state$1 = setValue(state$1, resource.url, resource.value);
...
export { afterRefetch, afterSet, beforeRefetch, beforeSet, command, conf, end, get, load, onGet, once, plugins, refresh, refreshRegExp, registerPlugin, resources, save, set, start, useOnGet, waitUntil };
